...
cd(handles.PathName); 
dlmwrite(analyzedDNA,M,'delimiter','\t','newline','pc','precision',10)
...
imgIndData(1,:)=Data(:,9);
handles.imgIndData=imgIndData(1,:);
...
if(AutoSaveVal==1)
[pathstr,DataName,ext] = fileparts(F2);
[StrRetnData,tData_FFE,tData_FFR]=SaveData(tData,xAData,FAData,handles.imgIndData,handles.ZeroRows);
StrRertnFileName=sprintf('%s-anlyzd.txt',DataName);
cd(PathNameData); 
dlmwrite(StrRertnFileName,StrRetnData,'delimiter','\t','newline','pc','precision',10);
...
dlmwrite(FfeFileName,tData_FFE,'delimiter','\t','newline','pc','precision',10);
case 3
FfrFileName=sprintf('%s-anlyzd-FFRtime.txt',DataName);    
dlmwrite(FfrFileName,tData_FFR,'delimiter','\t','newline','pc','precision',10);    
case 4
FfeFileName=sprintf('%s-anlyzd-FFEtime.txt',DataName);    
dlmwrite(FfeFileName,tData_FFE,'delimiter','\t','newline','pc','precision',10);
FfrFileName=sprintf('%s-anlyzd-FFRtime.txt',DataName);    
...
M=reshapeData([xeDNA;FeDNA],[xrDNA;FrDNA]);
cd(SaveFilePath); 
dlmwrite(FileName,M,'delimiter','\t','newline','pc','precision',10)
set(handles.display,'string','Analyzed DNA Only File was saved');
cd(handles.orgdir)
...
if(FilterIndex==1)
cd(SaveFilePath); 
dlmwrite(FileName,StrRetnData,'delimiter','\t','newline','pc','precision',10);
...
if(FilterIndex==1)  
cd(SaveFilePath); 
dlmwrite(FileName,tData_FFE,'delimiter','\t','newline','pc','precision',10);
cd(handles.orgdir)
else
...
if(FilterIndex==1)  
cd(SaveFilePath); 
dlmwrite(FileName,tData_FFR,'delimiter','\t','newline','pc','precision',10); 
cd(handles.orgdir)
else
    %timda data wasnt saved
    cd(handles.orgdir) 
...
[FAData,xAData]=AnalayzeData(bp,FData,xData,calibration,baseline,bL,stiffCoeff,xoffset);
[StrRetnData,tData_FFE,tData_FFR]=SaveData(tData,xAData,FAData,handles.imgIndData,handles.ZeroRows)
[df,dx] = OverLap(StrRetnData(:,3),StrRetnData(:,2),overlapVal);
xoffset=xoffset+dx;
